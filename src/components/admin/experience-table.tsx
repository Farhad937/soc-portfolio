...
import type { AdminExperienceRow } from "@/lib/admin-content";

export default function ExperienceTable({ entries }: { entries: AdminExperienceRow[] }) {
  const [query, setQuery] = useState("");

  const filtered = useMemo(() => {
    if (!query.trim()) return entries;
    const q = query.toLowerCase();
    return entries.filter((e) => e.company.toLowerCase().includes(q) || e.position.toLowerCase().includes(q));
  }, [query, entries]);
            ...
            {filtered.map((e) => {
              ...
              return (
                <tr key={e.id} className="border-b border-border last:border-0">
                  <td className="px-4 py-3 font-medium text-text">{e.company}</td>
                  <td className="px-4 py-3 text-text-muted">{e.position}</td>
