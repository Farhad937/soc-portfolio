...
import {
  ...
  setJourneyEntryStatus,
  reorderJourneyEntry,
} from "@/lib/admin-content/journey-mutations";

export default function JourneyRowActions({
  id,
  contentStatus,
  isFirst,
  isLast,
  ...
  function handleDelete() {
    if (!confirm("Delete this journey entry permanently? This cannot be undone.")) return;
    startTransition(() => deleteJourneyEntry(id));
  }

  return (
    <div className="flex items-center gap-1">
      <Link href={`/admin/journey/${id}`} className="rounded p-1.5 text-text-faint hover:bg-bg-raised hover:text-accent" title="Edit">
      ...
      {contentStatus === "published" ? (
        <button
          disabled={isPending}
          onClick={() => startTransition(() => setJourneyEntryStatus(id, "draft"))}
        ...
        <button
          disabled={isPending}
          onClick={() => startTransition(() => setJourneyEntryStatus(id, "published"))}
      ...
      {contentStatus !== "archived" && (
        <button
          disabled={isPending}
          onClick={() => startTransition(() => setJourneyEntryStatus(id, "archived"))}
      ...
      <button
        disabled={isPending}
        onClick={() => startTransition(() => duplicateJourneyEntry(id))}
      ...
      <button
        ...
        onClick={() => startTransition(() => reorderJourneyEntry(id, "up"))}
      ...
      <button
        ...
        onClick={() => startTransition(() => reorderJourneyEntry(id, "down"))}
        className="rounded p-1.5 text-text-faint hover:bg-bg-raised hover:text-text disabled:opacity-30"
        title="Move down"
      >
