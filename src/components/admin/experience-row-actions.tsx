...
import {
  ...
  setExperienceStatus,
  reorderExperience,
} from "@/lib/admin-content/experience-mutations";

export default function ExperienceRowActions({
  id,
  contentStatus,
  isFirst,
  isLast,
  ...
  function handleDelete() {
    if (!confirm("Delete this experience entry permanently? This cannot be undone.")) return;
    startTransition(() => deleteExperience(id));
  }

  return (
    <div className="flex items-center gap-1">
      <Link href={`/admin/experience/${id}`} className="rounded p-1.5 text-text-faint hover:bg-bg-raised hover:text-accent" title="Edit">
      ...
      {contentStatus === "published" ? (
        <button
          disabled={isPending}
          onClick={() => startTransition(() => setExperienceStatus(id, "draft"))}
        ...
        <button
          disabled={isPending}
          onClick={() => startTransition(() => setExperienceStatus(id, "published"))}
      ...
      {contentStatus !== "archived" && (
        <button
          disabled={isPending}
          onClick={() => startTransition(() => setExperienceStatus(id, "archived"))}
      ...
      <button
        disabled={isPending}
        onClick={() => startTransition(() => duplicateExperience(id))}
      ...
      <button
        ...
        onClick={() => startTransition(() => reorderExperience(id, "up"))}
      ...
      <button
        ...
        onClick={() => startTransition(() => reorderExperience(id, "down"))}
        className="rounded p-1.5 text-text-faint hover:bg-bg-raised hover:text-text disabled:opacity-30"
        title="Move down"
      >
